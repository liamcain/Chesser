...
export function parse_user_config(settings: ChesserSettings, content: string) {
  ...
  const user_config: ChesserConfig = {
    ...
    orientation:
      check_valid_value(parse_field(content, "orientation"), ORIENTATIONS) ??
      default_chesser_config.orientation,
    viewOnly:
      convert_boolean(parse_field(content, "viewOnly")) ??
      default_chesser_config.viewOnly,
    drawable:
      convert_boolean(parse_field(content, "drawable")) ??
      default_chesser_config.drawable,
    free: convert_boolean(parse_field(content, "free")) ?? default_chesser_config.free,
    pieceStyle:
      check_valid_value(parse_field(content, "pieceStyle"), PIECE_STYLES) ??
      default_chesser_config.pieceStyle,
    boardStyle:
      check_valid_value(parse_field(content, "boardStyle"), BOARD_STYLES) ??
      default_chesser_config.boardStyle,
  };
